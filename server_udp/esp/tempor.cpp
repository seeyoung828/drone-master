...
// [수정됨] 마스터에게 실제 남은 큐 길이 전달 및 통신 확인용 시각화
    bool send_beacon() {
        // 마지막의 "|0|" 부분을 "|남은 사진 장수|" 로 변경했습니다. (스케줄링 알고리즘 정상 작동용)
        String hdr = "BEACON|" + s_id + "|" + data_id + "|"
                   + String(total_chunks) + "|" + String(current_idx) + "|"
                   + String(image_queue.size()) + "|";
                   
        udp.beginPacket(DRONE_IP, DRONE_PORT);
        udp.write((const uint8_t*)hdr.c_str(), hdr.length());
        
        if (udp.endPacket() == 1) {
            // 마스터를 부를 때마다 화면에 '>' 를 찍어서 살아있음을 보여줍니다.
            Serial.print(">"); 
            return true;
        }
        Serial.println("\n[Net] Beacon send failed.");
        return false;
    }
    ...
    void run() {
        ...
        while (true) {
            ...
            // ── Wi-Fi 재연결 (Core 0 패닉 방지 적용) ──────────────
            if (WiFi.status() != WL_CONNECTED) {
                Serial.println("\n[WiFi] 재연결 중...");
                
                WiFi.reconnect(); // disconnect(true) 대신 안전한 reconnect() 사용
