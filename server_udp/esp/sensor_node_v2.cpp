...
public:
    ...
    String   sent_dir;
    String   state_file;
    String   esp_name;             // [추가] 설정된 노드 명칭
    String   mac_address;          // [추가] 하드웨어 MAC 주소
    ...
    SensorNode(const String& node_id, const String& img_dir = "/images")
        : s_id(node_id), images_dir(img_dir),
          ...
          esp_name("Apple_Tree_" + node_id), // [추가] 고유 이름 부여
          mac_address(""),
    ...
    float get_battery_voltage() {
        // [추가] ESP32 내장 ADC를 이용하여 전압을 측정하는 헬퍼 함수
        // GPIO 34 채널 입력값 기준 3.5V ~ 4.2V 범위를 백분율로 환산
        int analog_val = analogRead(34);
        float voltage = (analog_val / 4095.0) * 3.3 * 2; // 분배기 2배율 적용
    ...
    // --------------------------------------------------------
    bool send_beacon() {
        /** [수정됨] 확장된 비콘 송신: BEACON|S_ID|Data_ID|Total|Idx|Last_Flag|MAC|Name|Battery| */
        float batt = get_battery_voltage();
        String header = "BEACON|" + s_id + "|" + data_id + "|"
                      + String(total_chunks) + "|" + String(current_idx) + "|0|"
                      + mac_address + "|" + esp_name + "|" + String(batt, 1) + "|";
