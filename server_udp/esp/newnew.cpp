...
public:
    ...
    bool captureAndSave() {
        ...
        // [수정] 파일 크기 업데이트 (로그에 정확한 용량을 출력하기 위함)
        file_size = fb->len; 
...
void run() {
        ...
        while (true) {
            ...
            // ── [수정됨] 안전한 Wi-Fi 재연결 로직 ──
            // ── [수정됨] 확실한 Wi-Fi 재연결 로직 ──
            if (WiFi.status() != WL_CONNECTED) {
                Serial.println("\n[WiFi] 연결 끊김. 재연결 중...");
                
                WiFi.disconnect(); // 연결 해제만 함 (크래시 안 남)
                delay(1000);       // 모뎀 쉴 시간 부여
                
                // reconnect() 대신 확실하게 SSID와 비번을 다시 쏴줌
            ...
            // ──────────────────────────────────────────────────
            // [수정됨] 모든 사진 전송 완료 시 연결 완전 종료
            // ──────────────────────────────────────────────────
            if (image_queue.empty() && current_image_path.length() == 0) {
                Serial.println("\n===========================================");
                Serial.println(" [System] 10장 전송 성공! ");
                Serial.println(" [System] 임무를 완수하여 통신을 종료합니다.");
                Serial.println("===========================================\n");

                WiFi.disconnect(true); // 와이파이 모뎀 전원 차단

                // 무한 루프에 가둬서 더 이상 아무 동작도 하지 않게 만듦 (종료 효과)
                while(true) {
                    delay(10000); 
                }
            }
            ...
            while (true) {
                ...
                if (msg[0] == "GRANT") {
                    ...
                    // [Fix] target_idx >= total_chunks는 마스터가 모두 받았다는 의미이므로 완료 처리
                    // 단, current_idx도 total_chunks에 도달했을 때만 완료로 간주
                    if (target_idx >= total_chunks) {
                        Serial.printf("[Done] 마스터가 모든 청크 수신 확인 (target:%d >= total:%d)\n",
                                      target_idx, total_chunks);
                        is_finished = true;
                        break;
                    }
                    ...
                    // [Fix] 전송 후 current_idx가 total에 도달했으면 완료 플래그
                    if (current_idx >= total_chunks) {
                        Serial.println("[Done] 모든 청크 전송 완료. COMPLETE 준비.");
                        is_finished = true;
                        break;
                    }
                }
...
// ============================================================
// [디버깅용] 와이파이 연결 실패 상세 원인 분석기
// ============================================================
void WiFiStationDisconnected(WiFiEvent_t event, WiFiEventInfo_t info) {
    uint8_t reason = info.wifi_sta_disconnected.reason;
    Serial.printf("\n🚨 [WiFi 에러] 연결 실패! (하드웨어 코드: %d)\n", reason);
    
    switch (reason) {
        case 201: // WIFI_REASON_NO_AP_FOUND
            Serial.println(" ➡️ [원인] 'Drone_AP'라는 이름을 가진 와이파이가 주변에 없습니다.");
            Serial.println("    (해결: 파이 핫스팟이 꺼졌거나, 거리가 너무 멉니다.)");
            break;
        case 202: // WIFI_REASON_AUTH_FAIL
        case 15:  // WIFI_REASON_4WAY_HANDSHAKE_TIMEOUT
        case 2:   // WIFI_REASON_AUTH_EXPIRE
            Serial.println(" ➡️ [원인] 비밀번호가 틀렸거나, 암호화 방식(보안 설정)이 호환되지 않습니다.");
            Serial.println("    (해결: 파이의 hostapd.conf에서 WPA2/CCMP 설정 확인 요망.)");
            break;
        case 200: // WIFI_REASON_BEACON_TIMEOUT
            Serial.println(" ➡️ [원인] 공유기는 찾았는데, 신호가 너무 약해서 통신이 끊겼습니다.");
            Serial.println("    (해결: 안테나 불량이거나, 파이와 ESP32를 더 가까이 붙이세요.)");
            break;
        case 203: // WIFI_REASON_ASSOC_FAIL
            Serial.println(" ➡️ [원인] 공유기에서 연결을 튕겨냈습니다.");
            Serial.println("    (해결: MAC 주소 차단이 걸려있거나 파이 채널 설정 문제입니다.)");
            break;
        default:
            Serial.println(" ➡️ [원인] 기타 네트워크/전력 오류입니다. (충전기 전원 권장)");
            break;
    }
}
...
void setup() {
    ...
    if (!initCamera()) {
        ...
        // [핵심 해결책] 카메라 센서 안정화 대기 (2초) 및 더미 프레임 버리기
        Serial.println("[Camera] 센서 노출/화이트밸런스 안정화 대기 중...");
        delay(2000); 
        
        // 첫 사진은 보통 깨지거나 0바이트 오류가 나므로 한 장 찍어서 버림
        camera_fb_t* dummy = esp_camera_fb_get();
        if(dummy) esp_camera_fb_return(dummy);
    }

    // 2. 파일 시스템(LittleFS) 마운트 및 상태 로드
    node.begin();

    // 3. AP 연결 전 사진 10장 연속 촬영
    Serial.println("\n[Camera] 와이파이 연결 전 사전 촬영(10장)을 시작합니다...");
    for (int i = 0; i < 10; i++) {
        Serial.printf("[Camera] %d/10 번째 사진 촬영 중...\n", i + 1);
        node.captureAndSave();
        delay(1000); // 다음 촬영 전 센서 재충전 대기 (1초)
    }
    Serial.println("[Camera] 사전 촬영 완료!\n");
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // 와이파이가 연결된 후 안전하게 네트워크 포트 열기
        node.udp.begin(UDP_PORT); 
    } else {
        Serial.println("\n[!] Wi-Fi 연결 실패. 백그라운드 재시도.");
    }

    // 5. 방금 찍은 10장의 사진을 전송 대기열(큐)에 등록
    node._scan_images();
}
