...
v3.3 사양은 **다중 노드 환경의 세션 정합성**을 위해 고유 Data_ID 체계와 양방향 완료 확정(ACK) 메커니즘을 도입하였다.
...
| 필드명 | 설명 | 예시 |
| :--- | :--- | :--- |
| **Type** | 패킷 종류 (BEACON, GRANT, DATA, COMPLETE, ERROR, REVOKE, COMPLETE_ACK) | DATA |
...
- **Error Codes**
  ...
  - `LIVELOCK_PREVENT`: (v3.8) 동일 인덱스 반복 수신(30회 초과) 등으로 인한 진행 중단 방지. 마스터가 세션을 강제 종료함.

### 4.6 REVOKE (Master → Slave)
- **슬롯 권한 일시 회수 (Yield Slot)**: 마스터가 슬롯 점유 시간 초과 또는 최대 청크 수 도달 시 해당 노드에게 전송 중단을 명령한다. 슬레이브는 이 메시지를 받으면 즉시 전송 루프를 탈출하고 비콘 대기 상태로 복귀하되, **현재 세션 상태(Data_ID, Index)를 유지**하여 다음 `GRANT` 수신 시 중단된 지점부터 전송을 재개해야 한다.

### 4.7 COMPLETE_ACK (Master → Slave)
- **수집 확정**: 마스터가 체크섬 검증에 성공하고 최종 파일을 저장했음을 노드에게 알린다. 노드는 이 메시지를 수신한 후에만 현재 이미지를 전송 완료 처리(Sent 폴더 이동)한다.
