...
본 규격은 `scheduling_policy.md`의 **하이브리드 공정성 스케줄링(배터리 보존 및 단일 노드 독점 방지)** 정책을 기술적으로 구현하는 것을 목표로 한다.
...
### 5.2 슬롯 종료 및 재개 (Revoke & Resume)

- **전송 권한 회수 (Revoke)**: Master는 다음 조건 시 슬롯을 즉시 종료하고 다음 노드에게 권한을 부여한다.
  - 시간 제한 ($T$)
  - 청크 제한 ($N$)

- **세션 상태 보존 (Pause)**: 통신 중단 시 현재 상태를 DB에 저장하여 에너지 효율적인 수집을 보장한다. (Stateful Protocol)
  - `last_idx`
  - `last_contact_time`

- **이어받기 (Resume)**: 재접촉 시 Aging 반영 후 우선순위 상승 시 `last_idx + 1`부터 데이터 전송을 재개하도록 명령한다.
